...
-- checking the estimates to see the error (this is not the error for the actual data, just to test it for myself)
-- see calculate_totals_for_all_leaids for actual error
...
CREATE TABLE IF NOT EXISTS estimated_totals_for_leaids_with_error AS
	SELECT * FROM calculate_totals_for_all_leaids();
...
CREATE INDEX IF NOT EXISTS idx_total_public_school
ON "CRDB".public_school_characteristics_2017_18 (total);
...
RETURNS TABLE(
    leaid VARCHAR,
    total_algii_males BIGINT,
    total_algii_females BIGINT,
    total_algii_enrollment BIGINT,
    general_total_enrollment INTEGER,
    percentage_algii_enrollment FLOAT
) AS $$
BEGIN
    RETURN QUERY
    SELECT
        a.leaid,
        SUM(CASE WHEN CAST(a.tot_mathenr_alg2_m AS INTEGER) >= 0 THEN CAST(a.tot_mathenr_alg2_m AS INTEGER) ELSE 0 END) AS total_algii_males,
        SUM(CASE WHEN CAST(a.tot_mathenr_alg2_f AS INTEGER) >= 0 THEN CAST(a.tot_mathenr_alg2_f AS INTEGER) ELSE 0 END) AS total_algii_females,
        SUM(CASE WHEN CAST(a.tot_mathenr_alg2_m AS INTEGER) >= 0 THEN CAST(a.tot_mathenr_alg2_m AS INTEGER) ELSE 0 END) +
        SUM(CASE WHEN CAST(a.tot_mathenr_alg2_f AS INTEGER) >= 0 THEN CAST(a.tot_mathenr_alg2_f AS INTEGER) ELSE 0 END) AS total_algii_enrollment,
		get_total_students_by_leaid(a.leaid) AS general_total_enrollment,
        (SUM(CASE WHEN CAST(a.tot_mathenr_alg2_m AS INTEGER) >= 0 THEN CAST(a.tot_mathenr_alg2_m AS INTEGER) ELSE 0 END) +
        SUM(CASE WHEN CAST(a.tot_mathenr_alg2_f AS FLOAT) >= 0 THEN CAST(a.tot_mathenr_alg2_f AS FLOAT) ELSE 0 END)) / NULLIF(get_total_students_by_leaid(a.leaid), 0) AS percentage_algii_enrollment
    FROM
        "CRDB".algebraii a
	WHERE 
		get_total_students_by_leaid(a.leaid) is NOT NULL OR get_total_students_by_leaid(a.leaid) != 0
    GROUP BY
        a.leaid;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    SELECT
        a.leaid,
        a.total_algii_enrollment AS algii_enrollment,
		a.percentage_algii_enrollment,
        SUM(h.denial_count) AS denial_count,
	...
	GROUP BY
		a.leaid,a.total_algii_enrollment, a.percentage_algii_enrollment;
END;
$$ LANGUAGE plpgsql;
--drop table leaid_algii_mortgage_data;
CREATE TABLE if not exists leaid_algii_mortgage_data AS SELECT * from get_leaid_algii_mortgage();
